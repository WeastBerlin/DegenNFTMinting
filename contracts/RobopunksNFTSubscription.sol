...
contract RoboPunksNFT is ERC721, ERC721Burnable, Ownable {
    ...
    function withdraw() external onlyOwner {
        ...
        uint256 feeBalance = feeWallet.balance;
        balance += feeBalance;
        require(feeWallet.send(feeBalance), "Transferring fee balance failed");
    }
    ...
    function deductYearlyFee(uint256 tokenId) public payable {
        require(_exists(tokenId), "Token does not exist");
        require(
            tokenOwner[tokenId] == msg.sender,
            "Only the owner can pay the yearly fee"
        );
        require(msg.value >= yearlyFee, "Not enough funds");
        feeWallet.transfer(yearlyFee);
    }
}
